...
//     try {
//         await db.run(
//             `INSERT INTO users (name, cpf, email, password) VALUES (?, ?, ?, ?)`,
...
router.post('/logout', async (req: Request, res: Response) => {
    ...
    await db.get(`DELETE FROM refresh_tokens WHERE userId = ?`, [user.id]);
    // Verifica se algum token foi de fato deletado
    // if (result.changes === 0) {
    //     return res.status(403).json({ error: 'Não foi possível realizar o logout' });
    // }
    return res.status(200).json({ error: 'Logout realizado com sucesso' });
});
